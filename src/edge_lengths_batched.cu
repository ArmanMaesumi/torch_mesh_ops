...
// Each thread computes the edge lengths for one face in one batch.
template <typename scalar_t, typename index_t>
__global__ void edge_lengths_batched_kernel(
    const scalar_t* __restrict__ vertices, // (B, V, 3)
    const index_t*  __restrict__ faces,    // (B, F, 3)
    scalar_t* __restrict__ edge_lengths,   // (B, F, 3)
    const int64_t B,
    const int64_t V,
    const int64_t F
) {
  ...
  // batch and face index
  const int64_t b = idx / F;
  const int64_t f = idx % F;

  const int64_t face_offset = (b * F + f) * 3;
  const index_t v0 = faces[face_offset + 0];
  const index_t v1 = faces[face_offset + 1];
  const index_t v2 = faces[face_offset + 2];
  const int64_t v0_offset = (static_cast<int64_t>(b) * V + static_cast<int64_t>(v0)) * 3;
  const int64_t v1_offset = (static_cast<int64_t>(b) * V + static_cast<int64_t>(v1)) * 3;
  const int64_t v2_offset = (static_cast<int64_t>(b) * V + static_cast<int64_t>(v2)) * 3;
...
void edge_lengths_batched_cuda_forward(
  ...
  AT_DISPATCH_FLOATING_TYPES(vertices.scalar_type(), "edge_lengths_batched_cuda_forward", [&] {
    using scalar_t_ = scalar_t;
    if (faces.scalar_type() == at::kLong) {
      edge_lengths_batched_kernel<scalar_t_, int64_t>
      ...
      edge_lengths_batched_kernel<scalar_t_, int32_t>
        <<<blocks, threads, 0, stream.stream()>>>(
          vertices.data_ptr<scalar_t_>(),
...
torch::Tensor edge_lengths_batched(
    torch::Tensor vertices,  // (B, V, 3)
    torch::Tensor faces      // (B, F, 3)
) {
  ...
  // Allocate output tensor for edge lengths: (B, F, 3)
  torch::Tensor edge_lengths_out = torch::empty({B, F, 3}, options);

  edge_lengths_batched_cuda_forward(vertices, faces, edge_lengths_out);

  return edge_lengths_out;
}

void init_edge_lengths_batched(py::module &m) {
    m.def("edge_lengths_batched",
          &edge_lengths_batched,
          "Edge lengths (CUDA)");
  }
